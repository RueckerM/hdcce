...
eigenvectors $\widehat{U}_{(1)},\dots,\widehat{U}_{(p)}$. The number of factors is
estimated by

$$\widehat{K} = \#\{\, j : \hat{\lambda}_j / \hat{\lambda}_1 > \tau \,\},$$
...
and $\widehat{\boldsymbol{W}} = \bar{\boldsymbol{X}}\widehat{\boldsymbol{U}} \in \mathbb{R}^{T\times\widehat{K}}$,
the projection is

$$\widehat{\boldsymbol{\Pi}} = \boldsymbol{I}_T -
...
$$\widehat{\beta}_\lambda \in \arg\min_{b\in\mathbb{R}^p}
  \left\{ \frac{1}{nT}\sum_{i=1}^n \big\lVert \widetilde{Y}_i -
  \widetilde{\boldsymbol{X}}_i b \big\rVert^2 + \lambda \lVert b \rVert_1 \right\},$$
...
> `glmnet` minimises $(2nT)^{-1}\sum_i \lVert \cdot \rVert^2 + \lambda_g \lVert b \rVert_1$,
> so $\lambda = 2\lambda_g$. The `lambda` argument and the returned `Lambda` are
...
$$\widehat{\theta}_\kappa \in \arg\min_{\vartheta}
  \left\{\frac{1}{nT}\sum_{i=1}^n \big\lVert \widetilde{X}_{i(j)} -
  \widetilde{\boldsymbol{X}}_{i(-j)}\vartheta \big\rVert^2 +
  \kappa \lVert \vartheta \rVert_1 \right\},$$
...
$$\widehat{\beta}^{\text{de}}_j = \widehat{\beta}_{\lambda,j} +
  \frac{\sum_{i=1}^n \hat{u}_i^\top \hat{r}_i}
       {\sum_{i=1}^n \hat{u}_i^\top \widetilde{X}_{i(j)}}
  \qquad\text{and}\qquad
...
|---|---|---|
| `1` | $D^{-2}\,\hat{\sigma}^2\sum_i \lVert \hat{u}_i \rVert^2$, with $\hat{\sigma}^2 = \{n(T-\widehat{K})\}^{-1}\sum_i \lVert \hat{r}_i \rVert^2$ | homoscedastic, no serial correlation |
...
$$\widehat{\beta}_\lambda \in \arg\min_{b\in\mathbb{R}^d}
  \left\{ \frac{1}{nT}\sum_{i=1}^n \big\lVert \widetilde{Y}_i -
  \widetilde{\boldsymbol{\Phi}}_i b \big\rVert^2 + \lambda \lVert b \rVert_1 \right\},$$
...
$X_{it,j} = \sum_{j'\neq j}\phi^{\text{nw}}_{j'}(X_{it,j'})^\top\theta_{j'} + F_t^\top\nu_i + u_{it}$
with sparse $\theta$; a lasso on the projected data yields residuals
$\check{u}_i$. Supply `Phi_nw` and `group_nw` for a nodewise dictionary different
from `Phi`.

**(iv)** With $\check{\beta}_\lambda$ the lasso on
$(\check{Y}, \check{\boldsymbol{\Phi}})$ and
$R_i = \check{Y}_i - \check{\boldsymbol{\Phi}}_{i(-j)}\check{\beta}_{\lambda,-j}$ the
...
$$\Psi_{w,h} = \frac{\sum_{i=1}^n R_i^\top \tau_{w,h}(\check{u}_i)}
   {\big\{\sum_{i=1}^n \lVert \check{\boldsymbol{\Pi}}\tau_{w,h}(\check{u}_i) \rVert^2 \big\}^{1/2}},
  \qquad \Psi = \max_{w\in\mathcal{W}}\big|\Psi_{w,h}\big| .$$

**(v)** Critical values come from a Gaussian coupling. With

$$d_{i,w} = \frac{\check{\boldsymbol{\Pi}}\tau_{w,h}(\check{u}_i)}
  {\big\{(nT)^{-1}\sum_i \lVert \check{\boldsymbol{\Pi}}\tau_{w,h}(\check{u}_i) \rVert^2\big\}^{1/2}},
  \qquad
  \hat{\sigma}^2 = \frac{\sum_i \lVert \check{Y}_i - \check{\boldsymbol{\Phi}}_i\check{\beta}_\lambda \rVert^2}
                      {n(T-\widehat{K})},$$
...
test rejects when $\Psi \ge \hat{c}_{1-\alpha}$, the empirical $(1-\alpha)$
